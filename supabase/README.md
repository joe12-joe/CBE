...
5. Create your own users — **no demo accounts are shipped**:
   ...
   - Dashboard → **Authentication → Users → Add user** (email + password), then
     ...
     ```sql
     insert into public.profiles (id, name, email, role, school_ids, sub_county_ids, county_ids)
     ...
     - `SUB_COUNTY_ADMIN` — put sub-county uuid(s) in `sub_county_ids`.
...
- **RLS roles**: the `profiles` table stores `role`, `school_ids[]`,
  `sub_county_ids[]` and `county_ids[]`. Policies use security-definer helpers
  (`app_role()`, `can_access_school()`…) to scope every table to the signed-in
  user, so each role sees only its own data.
